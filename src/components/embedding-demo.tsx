...
import { useQueuedEmbeddingService, useEmbeddingModel } from '../lib/embedding-service';
import { getTursoDBService, MedicalSearchOptions } from '../lib/turso-db-service';
...
export default function EmbeddingDemo() {
  ...
  useEffect(() => {
    ...
    if (embeddingModel.error) {
      ...
      // Try to insert example documents if database is ready
      if (dbService.isReady()) {
        dbService.insertExampleDocumentsIfNeeded().catch(error => {
          console.error('Failed to insert example documents:', error);
        });
      }
    } else if (embeddingModel.isGenerating) {
      setModelStatus('Generating...');
    } else {
      setModelStatus(`Loading... ${Math.round(embeddingModel.downloadProgress * 100)}%`);
    }
  }, [embeddingModel.isReady, embeddingModel.error, embeddingModel.isGenerating, embeddingModel.downloadProgress]);
  ...
  const initializeServices = async () => {
    try {
      ...
      setDbStatus(`Database error: ${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      const documentResults = await dbService.searchMedicalDocuments(searchOptions);
  ...
  const insertExampleDocuments = async () => {
    try {
      setIsLoading(true);
      await dbService.insertExampleDocumentsIfNeeded();
      Alert.alert('Success', 'Example documents inserted successfully');
    } catch (error) {
      console.error('Failed to insert example documents:', error);
    ...
    <ScrollView style={{ flex: 1, padding: 16, backgroundColor: '#f5f5f5' }}>
      ...
      <View style={{ backgroundColor: '#fff', padding: 16, borderRadius: 8, marginBottom: 16 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
          <TouchableOpacity
            ...
            onPress={insertExampleDocuments}
            disabled={!embeddingModel.isReady || isLoading}
          >
            <Text style={{ color: '#fff', fontWeight: 'bold' }}>
            ...
            >
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: 12, color: '#007AFF', fontWeight: 'bold' }}>
      ...
      <View style={{ backgroundColor: '#fff', padding: 16, borderRadius: 8, marginTop: 16 }}>
        ...
        <Text style={{ fontSize: 14, color: '#666', lineHeight: 20 }}>
          This demo uses ExecuTorch's ALL-MiniLM-L6-v2 model to generate real text embeddings for semantic search. 
          The embeddings are used to find similar medical documents and Q&A pairs in the Turso vector database.
        </Text>
        <Text style={{ fontSize: 14, color: '#666', lineHeight: 20, marginTop: 8 }}>
          Try searching for medical topics like "heart disease", "diabetes treatment", or "cancer immunotherapy" 
          to see how the semantic search works with real embeddings.
        </Text>
      </View>
    </ScrollView>
  );
