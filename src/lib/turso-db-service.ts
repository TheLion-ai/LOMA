...
// EMBEDDING_DIMENSION now imported from embedding-service
...
export class TursoDBService {
  ...
  private async createTables(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "Database tables created successfully with new medical schema"
      );
    } catch (error) {
    ...
    try {
      ...
      const exampleDocs: MedicalDocument[] = [
        ...
        },
        {
          ...
          content:
            "Immunotherapy has revolutionized cancer treatment by harnessing the body's immune system to fight cancer cells. Checkpoint inhibitors, CAR-T cell therapy, and cancer vaccines represent major breakthroughs in oncology. These treatments have shown remarkable success in various cancer types including melanoma, lung cancer, and hematological malignancies.",
          vector: await this.generateEmbedding(
            "Immunotherapy has revolutionized cancer treatment by harnessing the body's immune system to fight cancer cells. Checkpoint inhibitors, CAR-T cell therapy, and cancer vaccines represent major breakthroughs in oncology. These treatments have shown remarkable success in various cancer types including melanoma, lung cancer, and hematological malignancies."
          ),
          created_at: new Date().toISOString(),
        ...
        },
        {
          ...
          content:
            "Type 2 diabetes management has been transformed by continuous glucose monitoring, insulin pumps, and mobile health applications. These technologies enable better glycemic control, reduce complications, and improve quality of life for patients. Telemedicine has also expanded access to specialized diabetes care.",
          vector: await this.generateEmbedding(
            "Type 2 diabetes management has been transformed by continuous glucose monitoring, insulin pumps, and mobile health applications. These technologies enable better glycemic control, reduce complications, and improve quality of life for patients. Telemedicine has also expanded access to specialized diabetes care."
          ),
          created_at: new Date().toISOString(),
        ...
        },
        {
          ...
          content:
            "Antimicrobial resistance poses a significant threat to global health security. Inappropriate antibiotic use, inadequate infection control, and lack of new drug development contribute to this crisis. Stewardship programs, rapid diagnostic tests, and international cooperation are essential to combat resistance.",
          vector: await this.generateEmbedding(
            "Antimicrobial resistance poses a significant threat to global health security. Inappropriate antibiotic use, inadequate infection control, and lack of new drug development contribute to this crisis. Stewardship programs, rapid diagnostic tests, and international cooperation are essential to combat resistance."
          ),
          created_at: new Date().toISOString(),
      ...
      const exampleQAs: MedicalQA[] = [
        {
          ...
          answer:
            "The main risk factors include hypertension, high cholesterol, diabetes, smoking, obesity, sedentary lifestyle, family history, and age. Many of these are modifiable through lifestyle changes and medical management.",
          vector: await this.generateEmbedding(
            "What are the main risk factors for cardiovascular disease?"
          ),
          document_id: "doc1",
        },
        {
          ...
          answer:
            'Checkpoint inhibitors block proteins that prevent immune cells from attacking cancer cells. By removing these "brakes" on the immune system, T-cells can better recognize and destroy cancer cells. Common targets include PD-1, PD-L1, and CTLA-4.',
          vector: await this.generateEmbedding(
        ...
        },
        {
          id: "qa3",
          question:
            "What is the target HbA1c level for most diabetic patients?",
          answer:
            "For most adults with diabetes, the target HbA1c level is less than 7%. However, individualized targets may vary based on age, comorbidities, life expectancy, and risk of hypoglycemia. Some patients may have targets of 6.5% or 8% depending on their specific circumstances.",
          vector: await this.generateEmbedding(
            "What is the target HbA1c level for most diabetic patients?"
          ),
          document_id: "doc3",
        },
        {
          id: "qa4",
          question:
            "What screening tools are commonly used for depression in primary care?",
          answer:
            "The PHQ-9 (Patient Health Questionnaire-9) and PHQ-2 are widely used screening tools. The GAD-7 is used for anxiety screening. These validated instruments help identify patients who may benefit from further mental health evaluation and treatment.",
          vector: await this.generateEmbedding(
            "What screening tools are commonly used for depression in primary care?"
          ),
          document_id: "doc4",
        },
        {
          ...
          answer:
            "Antibiotic stewardship involves coordinated interventions to improve antibiotic use, including prescribing the right drug, dose, and duration. It's crucial for reducing resistance, minimizing adverse effects, decreasing healthcare costs, and preserving antibiotic effectiveness for future generations.",
          vector: await this.generateEmbedding(
      ...
      try {
        ...
        console.log(
          `${exampleDocs.length} example documents and ${exampleQAs.length} Q&A pairs inserted successfully`
        );
      } catch (txError) {
        // Rollback on error
      ...
      throw new Error(
        `Failed to insert example documents: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
    }
