...
export class TransformersAIService implements AIService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Gemma 3n server is not running. Please start it with: node gemma-server.js"
        );
      }
      ...
      throw new Error(
        `Gemma 3n server connection failed: ${error.message}. Make sure to run: node gemma-server.js`
      );
    } finally {
  ...
  private async waitForModelReady(): Promise<void> {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        console.log(
          `🔄 Waiting for Gemma server to start... (${Math.round(
  ...
  async complete(options: AICompletionOptions): Promise<AICompletionResult> {
    ...
    try {
      ...
      console.error(
        "❌ Error communicating with Gemma 3n server:",
        error.message
      );
